...
/-- Over a reduced commutative ring in which `(r : R)` is a unit, `X ^ r - 1` is a radical
    element of `R[X]`. -/
lemma X_pow_sub_one_of_isUnit {R : Type*} [CommRing R] [IsReduced R] (r : ℕ)
    (hru : IsUnit (r : R)) : _root_.IsRadical ((X : R[X]) ^ r - 1) := by
...
lemma X_pow_sub_one_of_regular (h : IsRegular (r : R)) : (span {(X : R[X]) ^ r - 1}).IsRadical := by
...
lemma IsReduced.X_pow_sub_one_of_coprime (h : n.Coprime r) [IsReduced R] :
  ...
  apply IsRadical.X_pow_sub_one_of_regular
